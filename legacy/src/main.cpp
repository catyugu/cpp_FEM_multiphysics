...
int main() {
    ...
    simulation.assemble_initial_condition([](const Eigen::Vector3d& coord) {
        if (std::abs(coord.x()) < 0.005) return 80.0;
        return 20.0; // Initial temperature of 20 C
    });

    // simulation.assemble_boundary_condition([](const Eigen::Vector3d& coord) {
    ...
    simulation.run_simulation(10, 0.5);
