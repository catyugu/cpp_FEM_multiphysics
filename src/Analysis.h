...
public:
    ...
    void apply_initial_condition(const std::function<double(const Eigen::Vector3d& coord)>& init_func);
    void apply_boundary_condition(const std::function<double(const Eigen::Vector3d& coord)>& border_func);
...
private:
    ...
    HeatParams params; // For this example, params are stored here
