...
namespace Core {
    ...
    const std::vector<std::unique_ptr<Physics::PhysicsField>>& Problem::getFields() const { return fields_; }

    // --- Corrected Steady-State Solver ---
    void Problem::solveSteadyState() const {
        auto &logger = SimpleLogger::Logger::instance();
    ...
    void Problem::solveTransient() const {
        auto &logger = SimpleLogger::Logger::instance();
        ...
        if (emag_field_base && heat_field_base) {
            ...
            for (int i = 0; i < num_steps; ++i) {
                ...
                Eigen::SparseMatrix<double> A = (heat_field_base->getMassMatrix() / time_step_) + heat_field_base->
                                                getStiffnessMatrix();
                Eigen::MatrixXd b = heat_field_base->getRHS() + (heat_field_base->getMassMatrix() / time_step_) *
