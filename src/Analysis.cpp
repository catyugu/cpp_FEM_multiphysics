...
void Analysis::identify_border_nodes() { /* ... implementation is the same as before ... */ }
void Analysis::apply_initial_condition(const std::function<double(const Eigen::Vector3d&)>& init_func) {
    for (size_t i = 0; i < mesh->nodes.size(); ++i) {
        solution(i) = init_func(mesh->nodes[i].coordinates);
    }
}
void Analysis::apply_boundary_condition(const std::function<double(const Eigen::Vector3d&)>& border_func) {
    for (size_t i = 0; i < mesh->nodes.size(); ++i) {
        if (is_border_node[i]) {
