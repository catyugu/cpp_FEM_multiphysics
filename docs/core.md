Of course, here is the documentation for the Core, Physics, and IO namespaces.
...
* **Public Functions**:
    * `apply(Eigen::SparseMatrix<double>& K, Eigen::VectorXd& F) const`: Apply the boundary condition to the global system of equations.
* **Derived Classes**:
    * `DirichletBC`
    * `NeumannBC`
    * `CauchyBC`
...
* **Public Functions**:
    ...
    * `apply(Eigen::SparseMatrix<double>& K, Eigen::VectorXd& F) const override`
* **Private Members**:
    * `equation_index_`: `int`
    * `value_`: `double`
...
* **Public Functions**:
    * `solve(const Eigen::SparseMatrix<double>& A, const Eigen::VectorXd& b, Eigen::VectorXd& x)`: Solves the system Ax = b and returns x.
